...
export default function EditNameModal({
  ...
  currentName: string;
  currentUurtarief?: number;
  onSave: (naam: string, uurtarief?: number) => void;
  onClose: () => void;
}) {
  const [naam, setNaam] = useState(currentName);
  const [rateText, setRateText] = useState(String(currentUurtarief ?? ''));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-xl p-6 min-w-[340px]" onClick={e => e.stopPropagation()}>
        <h2 className="text-lg font-semibold mb-4">{LABELS[type]} bewerken</h2>
